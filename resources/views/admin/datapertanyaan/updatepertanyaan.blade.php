                    ...
                    <form class="col-lg-8" method="post" action="/datapertanyaan/update/{{ $data->id }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select form-control @error('kriteria_id') is-invalid @enderror" name="kriteria_id">
                                @foreach ($kriterias as $kriteria)
                                    @if (old('kriteria_id', $data->kriteria_id) == $kriteria->id)
                                        <option value="{{ $kriteria->id }}" selected>{{ $kriteria->kriteria }}</option>
                                    @else
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_pertanyaan') is-invalid @enderror" id="nama_pertanyaan"
                                name="nama_pertanyaan" required value="{{ old('nama_pertanyaan', $data->nama_pertanyaan) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select form-control @error('periode_id') is-invalid @enderror" name="periode_id">
                                @foreach ($periodes as $periode)
                                    @if (old('periode_id', $data->periode_id) == $periode->id)
                                        <option value="{{ $periode->id }}" selected>{{ $periode->bulan }}</option>
                                    @else
